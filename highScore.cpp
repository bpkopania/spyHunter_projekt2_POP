...
bool showScore(char mode, _screen screen)
{
	...
	if (scoresFile != NULL)
	{
		for (int i = 0; i < MAX_SIZE_HIGHSCORE; i++)
		{
			fscanf(scoresFile, "%i %lf ", &(ordered[i].score), &(ordered[i].time));
		}
		while (!end)
		{
...
char modePicker(_screen screen)
{
	...
	while (!end)
	{
		...
		sprintf_s(text, "Press t to show highscore ordered by time");
		DrawString(screen.screen, screen.screen->w / 2 - strlen(text) * 8 / 2, 36, text, screen.charset);
		sprintf_s(text, "Press p to show highscore ordered by points");
		...
		while (SDL_PollEvent(&(screen.event))) {
			...
			case SDL_KEYDOWN:
				if (screen.event.key.keysym.sym == SDLK_ESCAPE) end = true;
				else if (screen.event.key.keysym.sym == SDLK_p) return 's';
				else if (screen.event.key.keysym.sym == SDLK_t) return 't';
				break;
			case SDL_KEYUP:
				break;
			case SDL_QUIT:
